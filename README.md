...
- **同步防抖**：频繁编辑时会合并短时间内的多次变更，降低同步请求频率。
- **PWA 支持**：可安装到桌面/手机主屏幕，提升访问体验。
- **现代 UI 体验**：使用 Tailwind CSS 结合 Framer Motion 打造高级的玻璃拟态（Glassmorphism）和流畅的动效，支持亮色/暗色模式无缝切换。
- **沉浸式刷题**：
  - **FSRS 间隔重复算法**：根据您的掌握程度（未掌握、模糊、已掌握）动态安排下一次复习时间，复习节奏更贴近真实遗忘曲线。
  - 支持按“全部题目”、“智能复习”、“只看新题”分类刷题。
  - 3D 翻转卡片交互，提供纯粹的沉浸感。
- **AI 辅助解析（可选）**：支持配置 Qwen API，为题目生成 AI 参考答案；在答案卡片右上角可随时切换“固定答案 / AI 解析”。
- **强大的题库管理**：
  - 支持 Markdown 语法编写题目和解析，并接入了 `highlight.js` 实现代码高亮。
  - **批量操作**：支持对选中的题目进行批量修改难度、追加标签、批量删除。
  - **多样化导入导出**：支持 Excel (.xlsx, .xls)、CSV、TXT 格式批量上传；Excel/CSV 导出包含“答案与解析（固定答案）”与“AI答案”两列；支持 JSON 格式的整库备份与恢复，并提供了导入冲突解决策略（跳过、覆盖、生成新 ID）。
